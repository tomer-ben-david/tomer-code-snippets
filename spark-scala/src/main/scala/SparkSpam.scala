...
object SparkSpam extends App {

  override def main(args: Array[String]): Unit = {

    // http://raw.githubusercontent.com/datasciencedojo/IntroToTextAnalyticsWithR/master/spam.csv

    val spark = org.apache.spark.sql.SparkSession.builder().appName("someapp").master("local[*]").getOrCreate()

    import org.apache.spark.sql.functions._
    import spark.implicits._
//    val df = spark.read.option("header", "true").option("ESCAPE quote", '"')
//      .csv("file:///Users/tomer.bendavid/Downloads/spam.csv").toDF()

    val df = spark.read.format("csv").option("delimiter", ",").option("ESCAPE quote", '"'). option("header",true).option("encoding", "UTF-8")
      .load("file:///Users/tomer.bendavid/Downloads/spam.csv")
    ...
    val iris = arffParser.parse(this.getClass.getResourceAsStream("/iris.arff.txt"))
    smile.plot.plot(iris, '*', Array(Color.RED, Color.BLUE, Color.CYAN))
